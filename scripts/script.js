...
document.addEventListener('DOMContentLoaded', function () {
  const cardsContainer = document.querySelector('.mobile-artists-cards-row');
  const cards = document.querySelectorAll('.mobile-artists-card');
  let currentIndex = 0;
  const cardWidth = cards[0].offsetWidth + 20; // Ширина однієї карточки
  let interval;

  // Функція для прокрутки до наступної карточки
  function scrollToNextCard() {
      currentIndex = (currentIndex + 1) % cards.length; // Використовуємо оператор % для зациклення індексу
  ...
  function checkWindowWidth() {
      if (window.innerWidth > 1024) {
          ...
          clearInterval(interval); // Зупинити автоматичну прокрутку, якщо вікно менше або рівне 1024px
      }
  }

  // Викликати перевірку при завантаженні та зміні розміру вікна
  window.addEventListener('resize', function () {
      checkWindowWidth();
      currentIndex = 0; // Скинути індекс при зміні розміру вікна
  });

  // Опціонально: відключити автоматичну прокрутку при наведенні мишки на контейнер
  cardsContainer.addEventListener('mouseenter', function () {
      clearInterval(interval); // Зупинити автоматичну прокрутку
  });

  // Опціонально: увімкнути автоматичну прокрутку при виведенні мишки з контейнера
  cardsContainer.addEventListener('mouseleave', function () {
      checkWindowWidth(); // Перевірити ширину вікна та рішення щодо автоматичної прокрутки
  });

  // Перевірка ширини вікна при завантаженні
  checkWindowWidth();
});
